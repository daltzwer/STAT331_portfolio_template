...
#in this data set,the researcher was trying to identify whether they can predict musical taste based on their familiarity with African American English (AAE). In total, there were 38 variables that were being considered such as sex, age, and city. 
...
The rows of the data seems to represent 10,752 different AAE word/person combo.
...
Missing values were replaced by their mean. By filling in missing values, you get more data to work with to help us with our research. However, it is not a reflection of the actual idea but our best guess.
...
#there are 64 unique words in this dataset
```
...
```{r}
#collapsing ethnic/racial categories into "white and non-white" misrepresents the other racial groups that are being studied or surveyed. Addiotionally, this gives the unintentional message that the only ethnic group worth naming are "whites" as evidenced by the categorization of other racial groups as simply "non-white".
```
...
ggplot(data = hiphop_clean, 
  ...
  labs( x = "Sex of Participant", 
        ...
        title = "Average age of Participants by Gender")

```

```{r}
ggplot(data = hiphop_clean, 
       mapping = aes(x = ethnic1, 
                     y = age)) +
  geom_boxplot() +
  labs( x = "Ethnicity of Participant", 
        y = "Age of Participant", 
        title =  "Average age of Participants by Ethnicity")
...
According to the data, "off the hook" is the word/phrase people who are below the age 20 are most familiar with with an average score of 4.4237 and "catch the vapor" is the word/phrase poeple are the least familiar with with an average score of 1.0254.
...
According to the data above, non-white women are most familiar with the phrase "feel me" with an average score of 4.19 while break someone out, dukey rope, plex, and rollie were all tied for the phrases non-white women are most unfamiliar with with an average score of just 1.
...
According to the data above, there were a number of phrases that white men above 30 are unfamiliar with which includes, ay yo trip, beezy, and dap just to name a few. In total, there were 25 phrases that were tied that white men above 30 are unfamiliar with with an average score of just 1. Meanwhile, they were most familiar with the phrase 5-0, which had an average score of 4.2.
...
Intrestingly, we filtered according to the data and clues expecting the last person to score a 6 on the bieber song scale but there was no one. We did, however, find p17 who scored a 5 which leads us to believe that this must be Justin Bieber based on the data and hints provided above.
